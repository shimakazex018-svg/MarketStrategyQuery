...
function seedSyntheticPortfolio(db, now = new Date()) {
  ...
  withTransaction(db, () => {
    ...
    const performanceInsert = db.prepare('INSERT INTO daily_performance(account_key,date,begin_nav,end_nav,external_net_flow,pnl_amount,daily_return,cumulative_return,calculation_method,quality_status,reconciliation_difference,reported_pnl,reported_return,imported_at,total_cash,gross_position_value,flow_count) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
    for (const item of performance) performanceInsert.run(item.accountKey, item.date, item.beginNav, item.endNav, item.externalNetFlow, item.pnlAmount, item.dailyReturn, item.cumulativeReturn, item.calculationMethod, item.qualityStatus, item.reconciliationDifference, item.reportedPnl, item.reportedReturn, item.importedAt, item.totalCash, item.grossPositionValue, item.flowCount);
