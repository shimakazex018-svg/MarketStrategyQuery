...
| CFTC Nasdaq期货仓位 | CFTC官方TFF Futures Only数据集存在机器接口；`209742`（E-mini Nasdaq-100）已选为正式代理，`209747`（Micro E-mini Nasdaq-100）仅作诊断候选 | 检查点B已实现官方机器接口、周六调度、缓存与错误分类；运行数据不提交Git |

2026-07-15本轮尝试读取正式CFTC数据行时，受限执行环境返回网络失败；申请非沙箱重试又因工具额度限制未获批准。因此当前结论是“适配器和官方字段已验证、真实运行行尚未在本机成功缓存”，不能将卡片标记为`fresh`。
...
6. CFTC官方资料和机器接口字段已确认`209742`和`209747`；正式序列选择`209742`，`209747`不进入正式卡片。
7. 检查点B允许在用户完成本地配置后受控下载SEC companyfacts/bulk；CFTC TFF已获批使用`209742`。QQQ持仓/权重和价格不得自动下载。
