...
### Impact
MAD为0、有效样本少于20、边界不可计算或非有限输入时必须标记`robust_method_unavailable`，不得回退固定阈值。正式全量阶段至少80个有效成分；受影响权重超过10%时结果仅供诊断并标记`quality_warning`。未来首页与PE有关的正式卡只显示原始与稳健两种自计算结果，且不得宣称等同或复制Invesco、Nasdaq、WorldPEratio指标。
